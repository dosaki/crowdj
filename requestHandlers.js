...
function addLink(response, getData) {
	...
	if(validateURL(getData.lnk)) {
		...
		if(hostname.indexOf("youtube") !== -1){
			...
			exec("echo " + video.v + " >> priorityList", function (error, stdout, stderr) {});
		}
	}
	else{
...
function play(response) {
	...
	shell.exec("head -n 1 priorityList", function (error, stdout, stderr) {
		var playlist = "None";
		if(stdout == "")
			playlist = "songList";
		else
			playlist = "priorityList";
		...
		if(playlist == "songList"){
			shell.exec("echo `head -n 1 songList` >> songList", function (error, stdout, stderr) {
